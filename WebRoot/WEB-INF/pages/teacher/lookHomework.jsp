					   		...
					   		<div id="collapseOne" class="panel-collapse collapse in">
					    		<div class="panel-body">
					        		<div style="font-size:14px;">
				    					...
				    					<a href="logout"><input type="button" value="注销"  style="margin-right:2%;float:right;width:100px;"/></a>
					        		</div>
					      		</div>
					    	</div>
					  </div>
								  ...
								  <c:forEach   items="${sclist}" var="s">
									<tr style="overflow:hidden;text-overflow:ellipsis;">
										...
										<th style="width: 10%;">
												...
												<input type="checkbox" id="ss" name="checkbox" ${s.allowlook } onclick="showcheck(${s.sno},${s.cno},${s.sctimes});"/>
										</th>
        			...
        			<div  style=" width: 685px; border-bottom:2px dashed #53A3C7;" >
       					<div class="left"   style="width: 25%; float:left;margin-left:10px">共${scNum}条记录</div>
       	    				<div class="right"  style=" folat:right">
				        		...
				        		<c:if test="${currentPage != 1}">
				            		<a href="teacher/lookHomework/${course.cno}?page=${currentPage-1}"> << 前一页</a>
				       			</c:if>
				        		<c:if test="${currentPage == 1}">
				        			<span class="current">1</span>
				       			</c:if>
				        		<c:if test="${currentPage != 1}">
				        			<a href="teacher/lookHomework/${course.cno}?page=1">1</a>
				        		</c:if>
				        		<%
				        		...
				        		<c:if test="${currentPage != page}">
				            	 	<a href="teacher/lookHomework/${course.cno}?page=<%=i+1%>"><%=i+1%></a>
				        		</c:if>
				        		<%} %>
				        		...
				        		<c:if test="${currentPage != pageTimes}">
				        			<a href="teacher/lookHomework/${course.cno}?page=${currentPage+1}">后一页  >></a>
				        		</c:if>
				        	 </div>
				        </div>
				     </div> 
