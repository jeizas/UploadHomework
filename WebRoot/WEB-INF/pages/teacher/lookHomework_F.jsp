					  ...
					  <div class="panel-group" id="accordion" style="margin-top:0;">
					 	...
					 	<div class="panel panel-default">
					    	...
					    	<div id="collapseFive" class="panel-collapse collapse">
					      		<div class="panel-body">
					        		<div>
					       			<form action="teacher/allowlook/${course.cno} " method="post">
					       				<span>班级</span>
					       				<select name="sclass" style="width:85%;">
					       					<c:forEach items="${ sclass}" var="c">
					       						<option value="${c}">${c}班</option>
					       				...
					       				<input type="submit" value="允许看" style="margin-right:2%;float:right;width:100px;"/>
					       			</form>
					       		</div>
					      		</div>
					    	</div>
        			...
        			<div  style=" width: 685px; border-bottom:2px dashed #53A3C7;" >
       					<div class="left"   style="width: 25%; float:left;margin-left:10px">共${scNum}条记录</div>
       	    				<div class="right"  style=" folat:right">
				        		...
				        		<c:if test="${currentPage != 1}">
				            		<a href="teacher//select/{cno}?page=${currentPage-1}"> << 前一页</a>
				       			</c:if>
				        		<c:if test="${currentPage == 1}">
				        			<span class="current">1</span>
				       			</c:if>
				        		<c:if test="${currentPage != 1}">
				        			<a href="teacher//select/{cno}?page=1">1</a>
				        		</c:if>
				        		<%
				        		...
				        		<c:if test="${currentPage != page}">
				            	 	<a href="teacher//select/{cno}page=<%=i+1%>"><%=i+1%></a>
				        		</c:if>
				        		<%} %>
				        		...
				        		<c:if test="${currentPage != pageTimes}">
				        			<a href="teacher//select/{cno}page=${currentPage+1}">后一页  >></a>
				        		</c:if>
				        	 </div>
				        </div>
				     </div> 
